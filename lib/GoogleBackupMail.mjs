      ...
      while (offsetUid < newestUid) {
        ...
        for (const message of Object.values(messages)) {
          try {
            ...
            const filepathFile = path.join(this.filepathByID, `${message.attributes['x-gm-msgid']}.eml`);

            // Write .eml file (if it doesn't exist yet)
            if (await fse.pathExists(filepathFile) === false) {
            ...
            // Link from 'By Label'
            await fse.symlink(filepathFile, path.join(boxFilepathByLabel, `${message.attributes.uid}.eml`));

            // Link from 'By Thread'
            const threadId = String(message.attributes['x-gm-thrid']);
            if (threadId) {
              await fse.ensureDir(path.join(this.filepathByThread, threadId));
              await fse.symlink(filepathFile, path.join(this.filepathByThread, threadId, `${message.attributes['x-gm-msgid']}.eml`));
            }
